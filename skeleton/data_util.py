...
class CNN_Data(Dataset):
    ...
    def __init__(self, csv_dir):
        ...
        self.MRI_data = []
        self.label = []
        self.sample_filepaths, sample_labels = read_csv(csv_dir)
        for filepath in self.sample_filepaths:
            self.MRI_data.append(torch.from_numpy(np.load(filepath)))
    ...
    def __getitem__(self, idx):
        ...
        '''
        # YOUR CODE HERE
        MRI_filename = Path(self.sample_filepaths[idx]).stem
        return self.MRI_data[idx], MRI_filename, self.label[idx]
...
def split_csv(csv_file, output_folder='../data/Assignment_3_data/ADNI3/', random_seed=1051):
    '''
    Attributes:
    ...
    for index, row in df.iterrows():
        complete_path = '..' + row['path'] + row['filename']
        if Path(complete_path).exists():
            MRI_metadata_series.append(row)

    # Randomly select the MRIs for test and background datasets
    random.seed(random_seed)
    random.shuffle(MRI_metadata_series)
    # Test dataset
    pd.DataFrame(MRI_metadata_series[12:]).to_csv(os.path.join(output_folder, r'test_MRI_metadata.csv'), index=False)
    # Background dataset
    pd.DataFrame(MRI_metadata_series[:12]).to_csv(os.path.join(output_folder, r'background_MRI_metadata.csv'),
                                                  index=False)
...
def read_csv(filename):
    ...
    for index, row in df.iterrows():
        complete_path = '..' + row['path'] + row['filename']
        sample_filepaths.append(complete_path)
        sample_labels.append(row['AD'])
    return sample_filepaths, sample_labels
